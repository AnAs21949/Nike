import PopulerProductCard from "../Components/PopulerProductCard";
import { products } from "../constants";

export default function PopulerProducts() {
  return (
    <section id="products" className="max-container max-sm:mt-12">
       <div className="flex flex-col justify-start gap-5">
          <h2 className="text-4xl">Populer products</h2>
          <p className="lg:max-w-lg mt-2 font-montserrat text-slate-gray"
          > Our <span className="text-coral-red">Popular</span> Lorem ipsum dolor sit amet consectetur adipisicing elit. Quasi at facere cupiditate beatae soluta non quos quas reiciendis dolore eos?</p>
       </div>
       <div className="mt-16 grid lg:grid-cols-4 md:grid-cols-3
       sm:grid-cols-2 grid-cols-1 sm:gap-4 gap-14">
        {products.map((product) => (
          <PopulerProductCard key={product.name} {...product}
          />
        ))}
       </div>
    </section>
  )
}
